...
public class WebConfiguredDynamicLiftSource implements DynamicLiftSource {
  ...
  public DynamicLifts select(Database db) throws Exception {
    ...
    Lift squat =
        findByName(
            squats,
            values.getOrDefault("deSquat", defaults.squat().lift().name()),
            defaults.squat().lift());
    Lift deadlift =
        findByName(
            deadlifts,
            values.getOrDefault("deDeadlift", defaults.deadlift().lift().name()),
            defaults.deadlift().lift());
    Lift bench =
        findByName(
            benches,
            values.getOrDefault("deBench", defaults.bench().lift().name()),
            defaults.bench().lift());
    Lift overhead =
        findByName(
            overheads,
            values.getOrDefault("deOverhead", defaults.overhead().lift().name()),
            defaults.overhead().lift());

    AccommodatingResistance squatAr = parseAr(values.get("deSquatAr"), defaults.squat().ar());
    AccommodatingResistance deadliftAr =
        parseAr(values.get("deDeadliftAr"), defaults.deadlift().ar());
    AccommodatingResistance benchAr = parseAr(values.get("deBenchAr"), defaults.bench().ar());
    AccommodatingResistance overheadAr =
        parseAr(values.get("deOverheadAr"), defaults.overhead().ar());
