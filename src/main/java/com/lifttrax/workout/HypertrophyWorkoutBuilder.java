...
public class HypertrophyWorkoutBuilder implements WorkoutBuilder {
    private static WorkoutLift single(String section, Lift lift, int reps, Integer percent, Float rpe) {
        return new WorkoutLift(section, new WorkoutLiftKind.SingleKind(
                new SingleLift(lift, new SetMetric.Reps(reps), percent, rpe, null, false)
        ));
    }

    private static List<WorkoutLift> repeated(String section, Lift lift, int sets, int reps, Integer percent, Float rpe) {
        List<WorkoutLift> setLifts = new ArrayList<>();
        for (int i = 0; i < sets; i++) {
            setLifts.add(new WorkoutLift(section, new WorkoutLiftKind.SingleKind(
                    new SingleLift(lift, new SetMetric.Reps(reps), percent, rpe, null, false)
            )));
        }
        return setLifts;
    ...
    private static Lift accessory(Database db, Muscle muscle) throws Exception {
        return require(db.getAccessoriesByMuscle(muscle), "missing accessory lift for muscle " + muscle.name());
    }
    ...
    private static Workout lowerDay(Database db, int week) throws Exception {
        ...
        lifts.add(new WorkoutLift("Warm-up", new WorkoutLiftKind.CircuitKind(
                new CircuitLift(List.of(new SingleLift(require(db.liftsByRegionAndType(LiftRegion.LOWER, LiftType.MOBILITY), "missing lower mobility lift"), new SetMetric.Reps(8), null, null, null, false)), 2, true)
        )));
    ...
    private static Workout upperDay(Database db, int week) throws Exception {
        ...
        List<WorkoutLift> lifts = new ArrayList<>();
        lifts.add(new WorkoutLift("Warm-up", new WorkoutLiftKind.CircuitKind(
                new CircuitLift(List.of(new SingleLift(require(db.liftsByRegionAndType(LiftRegion.UPPER, LiftType.MOBILITY), "missing upper mobility lift"), new SetMetric.Reps(8), null, null, null, false)), 2, true)
        )));
        lifts.addAll(repeated("Primary Hypertrophy", primary, 4, 8, 70, 7.5f));
        lifts.addAll(repeated("Secondary Hypertrophy", secondary, 3, 10, 65, 7.0f));
