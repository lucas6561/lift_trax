...
public record DynamicLifts(
    DynamicLift squat, DynamicLift deadlift, DynamicLift bench, DynamicLift overhead) {
  public static DynamicLifts fromDatabase(Database db) throws Exception {
    return fromDatabase(db, true, RandomSupport.DEFAULT);
  }

  public static DynamicLifts fromDatabase(Database db, boolean allowInteractiveSelection)
      throws Exception {
    return fromDatabase(db, allowInteractiveSelection, RandomSupport.DEFAULT);
  }

  static DynamicLifts fromDatabase(
      Database db, boolean allowInteractiveSelection, RandomSupport.Randomizer randomizer)
      throws Exception {
    Random random = new Random();
    AccommodatingResistance[] arOpts = {
    ...
    var defaults =
        new DynamicLiftSelector.DynamicLiftChoices(
            squatOptions.get(0),
  ...
  private static List<Lift> liftsForType(Database db, LiftType liftType, String fallbackName)
      throws Exception {
