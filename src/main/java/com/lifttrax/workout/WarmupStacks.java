...
public class WarmupStacks {
  ...
  public WarmupStacks(Database db, RandomSupport.Randomizer randomizer) throws Exception {
    List<Lift> lowerAccessoryLifts =
        new ArrayList<>(db.liftsByRegionAndType(LiftRegion.LOWER, LiftType.ACCESSORY));
    lowerAccessoryLifts.removeIf(
    ...
    List<Lift> upperAccessoryLifts =
        new ArrayList<>(db.liftsByRegionAndType(LiftRegion.UPPER, LiftType.ACCESSORY));
    upperAccessoryLifts.removeIf(
        l -> l.muscles().contains(Muscle.FOREARM) || l.muscles().contains(Muscle.CORE));
    ...
    lowerMobility =
        new RandomStack<>(db.liftsByRegionAndType(LiftRegion.LOWER, LiftType.MOBILITY), randomizer);
    if (lowerMobility.isEmpty()) {
      throw new IllegalArgumentException("not enough mobility lifts available");
    }

    upperMobility =
        new RandomStack<>(db.liftsByRegionAndType(LiftRegion.UPPER, LiftType.MOBILITY), randomizer);
  ...
  public WorkoutLift warmup(LiftRegion region) {
    ...
    RandomStack<Lift> accessoryStack =
        region == LiftRegion.LOWER ? lowerAccessories : upperAccessories;
    ...
    return new WorkoutLift(
        "Warmup Circuit", new WorkoutLiftKind.CircuitKind(new CircuitLift(warmupLifts, 3, true)));
  }
}
