...
- [ ] Documentation points users at the stable latest planned-workout schema
      path instead of a version-numbered schema snapshot.
- [ ] The guide includes a copyable prompt that asks an AI tool for JSON only,
      in accordance with the LiftTrax planned-workout schema.
- [ ] The prompt asks users to provide athlete goal, schedule, duration,
      available equipment, available exercise names when known, and constraints.
- [ ] The guide explains how to validate or sanity-check the generated file
      before import.
- [ ] The guide links to at least one valid planned-workout example.
- [ ] README or another discoverable top-level doc links to the guide.
