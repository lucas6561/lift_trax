...
public class WebConfiguredMaxEffortPlanSource implements MaxEffortPlanSource {
    ...
    public MaxEffortPlan selectPlan(Database db, MaxEffortLiftPools pools) throws Exception {
        ...
        List<MaxEffortPlan.DeloadLowerLifts> lowerDeload = new ArrayList<>(MaxEffortPlan.deriveLowerDeloadFromPlan(lower));
        List<MaxEffortPlan.DeloadUpperLifts> upperDeload = new ArrayList<>(MaxEffortPlan.deriveUpperDeloadFromPlan(upper));

        for (int i = 0; i < lowerDeload.size(); i++) {
            int n = i + 1;
            MaxEffortPlan.DeloadLowerLifts def = lowerDeload.get(i);
            Lift squat = findByName(squats, values.getOrDefault("meLowerDeload" + n + "Squat", def.squat().name()), def.squat());
            Lift deadlift = findByName(deadlifts, values.getOrDefault("meLowerDeload" + n + "Deadlift", def.deadlift().name()), def.deadlift());
        ...
        for (int i = 0; i < upperDeload.size(); i++) {
            ...
            Lift bench = findByName(benches, values.getOrDefault("meUpperDeload" + n + "Bench", def.bench().name()), def.bench());
            Lift overhead = findByName(overheads, values.getOrDefault("meUpperDeload" + n + "Overhead", def.overhead().name()), def.overhead());
