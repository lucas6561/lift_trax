...
pub fn edit_max_effort_plan(
    ...
    if let Err(err) = eframe::run_native(
        ...
        Box::new(move |cc| {
            install_editor_fonts(&cc.egui_ctx);
...
fn install_editor_fonts(ctx: &egui::Context) {
    let mut fonts = FontDefinitions::default();
    fonts.font_data.insert(
        "max_effort_ui".to_string(),
    ...
    fonts
        ...
        .or_default()
        .insert(0, "max_effort_ui".to_string());
    fonts
        .families
    ...
    fn selection_row(&mut self, ui: &mut egui::Ui, idx: usize) {
        ...
        ui.push_id((self.id_prefix, idx), |ui| {
            ui.horizontal(|ui| {
                ...
                if ui
                    .add_enabled(idx + 1 < selection_len, egui::Button::new("▼").small())
                    .clicked()
                {
                    move_down = true;
                }
...
impl App for MaxEffortEditorApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.label(
                "Adjust the lift used for each week's max effort day. Each column represents the lifts that rotate every other week—use the ▲/▼ buttons to reorder within a column.",
            );
            ui.add_space(12.0);
