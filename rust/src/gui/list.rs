                            ...
                            |ui, metric_mode| {
                                ui.horizontal(|ui| match metric_mode {
                                    ...
                                    MetricMode::RepsLr => {
                                        ui.label("Reps:");
                                        ui.label("L:");
                                        ui.text_edit_singleline(&mut self.edit_reps_left);
                                        ui.label("R:");
                                        ui.text_edit_singleline(&mut self.edit_reps_right);
                                    }
                                    MetricMode::Time => {
                                        ui.label("Seconds:");
                                        ui.text_edit_singleline(&mut self.edit_reps);
        ...
        if let Some(first) = exec.sets.first() {
            ...
            match first.metric {
                SetMetric::Reps(r) => {
                    ...
                    self.edit_reps_left.clear();
                    self.edit_reps_right.clear();
                }
                SetMetric::RepsLr { left, right } => {
                    self.edit_metric_mode = MetricMode::RepsLr;
                    self.edit_reps_left = left.to_string();
                    self.edit_reps_right = right.to_string();
                ...
                ui.group(|ui| {
                    ...
                    for (lift_idx, exec_idx) in entries {
                        ...
                        ui.horizontal(|ui| {
                            ...
                            if ui.button("Edit").clicked() {
                                self.begin_exec_edit(lift_idx, exec_idx);
                            }
                            if let Some(id) = exec.id {
                                if ui.button("Delete").clicked() {
                                    if let Err(e) = self.db.delete_lift_execution(id) {
                        ...
                        if self.editing_exec == Some((lift_idx, exec_idx)) {
                            execution_form(
                                ui,
                                "edit",
                                &mut self.edit_weight_mode,
    ...
    fn save_exec_edit(&mut self) {
        if let Some((lift_idx, exec_idx)) = self.editing_exec {
            ...
            let metric = match self.edit_metric_mode {
                ...
                MetricMode::RepsLr => {
                    let left: i32 = match self.edit_reps_left.parse() {
                        Ok(r) => r,
                        Err(_) => {
                    ...
                    let right: i32 = match self.edit_reps_right.parse() {
                        Ok(r) => r,
                        Err(_) => {
